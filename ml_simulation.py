...
# ── Combat gains ──────────────────────────────────────────────────────────
COMBAT_VEL_P         = 7.0
COMBAT_VEL_MAX_ACC   = 19.0
COMBAT_MAX_TILT      = 0.50
COMBAT_ATT_P         = 14.0
COMBAT_ATT_MAX_RATE  = 9.0
COMBAT_RATE_P        = 0.22
COMBAT_EVASION_SPEED = 26.0

# ── Emergency safety ──────────────────────────────────────────────────────
EMERGENCY_RADIUS   = 1.5
EMERGENCY_SPEED    = 14.0
EMERGENCY_THR      = 0.78
EMERGENCY_MAX_TILT = 0.38

# ── Attitude recovery ─────────────────────────────────────────────────────
RECOVERY_TILT_THRESHOLD = np.radians(40)
RECOVERY_THR            = 0.72
...
class MLDroneArena:
    ...
    def __init__(self, gui=True):
        ...
        ctrl_input_dim = SEQ_LEN * (9 + N_OBS_CTRL * 6) + 10 * 9  # 3180 + 90 = 3270
        self.controller = DroneController(input_dim=ctrl_input_dim)
    ...
    def _create_obstacles(self):
        ids    = []
        colors = [[1.0, 0.3, 0.3, 1], [0.3, 0.8, 1.0, 1],
                  [1.0, 0.6, 0.2, 1], [0.8, 0.3, 1.0, 1]]
    ...
    def _closest_obs_for_controller(self, obs_states, drone_pos):
        if len(obs_states) == 0:
            return np.zeros((N_OBS_CTRL, 6), dtype=np.float32)
        dists = np.linalg.norm(obs_states[:, :3] - drone_pos, axis=1)
        return obs_states[np.argsort(dists)[:N_OBS_CTRL]].copy()
    ...
    def _launch_next_obstacle(self, drone_pos, drone_vel):
        ...
        self._next_launch_time = time.time() + 2.5
    ...
    def _drive_active_obs(self):
        ...
        for oid, target in list(self._active_obstacles.items()):
            ...
            if dist < 0.5:
                ...
                continue
            p.resetBaseVelocity(oid, ((to_target / dist) * speed).tolist(), [0, 0, 0])
        for oid in done:
    ...
    def _apply_wrench(self, base_thr, m_roll, m_pitch, m_yaw, dt):
        ...
        f             = MAX_THRUST / 4.0
        thrust_total  = float(np.sum(self.motor_thr)) * f
        tau_roll      = float((self.motor_thr[0]+self.motor_thr[2])
                             -(self.motor_thr[1]+self.motor_thr[3])) * f * ARM_LEN
        tau_pitch     = float((self.motor_thr[2]+self.motor_thr[3])
                             -(self.motor_thr[0]+self.motor_thr[1])) * f * ARM_LEN
        tau_yaw       = float((self.motor_thr[1]+self.motor_thr[2])
                             -(self.motor_thr[0]+self.motor_thr[3])) * f * 0.06
    ...
    def _attitude_to_wrench(self, roll, pitch, yaw, ang_vel, dt,
                            roll_des, pitch_des, yaw_rate_des, base_thr,
                            max_tilt=None, att_p=None, att_max_rate=None, rate_p=None):
        ...
        roll_rate_des  = np.clip(roll_rate_des,
                                 self.prev_rate_des[0] - SLEW,
                                 self.prev_rate_des[0] + SLEW)
        pitch_rate_des = np.clip(pitch_rate_des,
                                 self.prev_rate_des[1] - SLEW,
                                 self.prev_rate_des[1] + SLEW)
    ...
    def _compute_evasion(self, drone_pos, drone_vel, obs_states, dt):
        ...
        for obs in obs_states:
            ...
            closing    = -np.dot(threat_dir, obs_vel - drone_vel)
            ...
            if urgency > best_urgency:
                ...
                horiz = horiz / hlen if hlen >= 0.1 else np.array([1.0, 0.0, 0.0])
                perp_a = np.array([-horiz[1],  horiz[0], 0.0])
                perp_b = np.array([ horiz[1], -horiz[0], 0.0])
                obs_h  = np.array([obs_vel[0], obs_vel[1], 0.0])
                perp   = perp_a if (-np.dot(perp_a, obs_h) >=
                                    -np.dot(perp_b, obs_h)) else perp_b
        ...
        for dim in [0, 1]:
            d_pos = ARENA_HALF - drone_pos[dim]
            d_neg = drone_pos[dim] - (-ARENA_HALF)
            if d_pos < BOUNDARY_BUFFER:
                boundary_vel[dim] -= BOUNDARY_STRENGTH / (d_pos + 0.1)**2
            if d_neg < BOUNDARY_BUFFER:
                boundary_vel[dim] += BOUNDARY_STRENGTH / (d_neg + 0.1)**2
        if drone_pos[2] < FLOOR + BOUNDARY_BUFFER:
            boundary_vel[2] += BOUNDARY_STRENGTH / ((drone_pos[2] - FLOOR) + 0.1)**2
        elif drone_pos[2] > CEIL - BOUNDARY_BUFFER:
        ...
        return final_evade * EVASION_SPEED, best_urgency, boundary_vel

    def _predict_future(self, drone_history, obs_history):
        if self.predictor is None or len(drone_history) < SEQ_LEN:
            return None
        try:
            drone_seq = np.array(drone_history[-SEQ_LEN:], dtype=np.float32)
            obs_seq   = np.array(obs_history[-SEQ_LEN:],   dtype=np.float32)
            pred      = self.predictor.predict(drone_seq[np.newaxis],
                                               obs_seq[np.newaxis])
            return pred[0]  # (10, 9)
        except Exception:
            return None
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self._trail_positions) >= 2:
                fade = len(self._trail_positions) / 60
            ...
            # ── Predicted obstacle path overlay ───────────────────────────
            ...
            if is_emergency:
                ...
                if not (np.isfinite(m_roll) and np.isfinite(m_pitch)
                        and np.isfinite(m_yaw) and np.isfinite(base_thr)):
            ...
            # ==============================================================
            # PRIORITY 3 — Normal flight (waypoint + ML + evasion)
            # ==============================================================
            ...
            # ── ML controller (uses past states + GRU prediction) ──────────
            if len(self.drone_history) == SEQ_LEN:
                x_past = np.concatenate([
                    np.concatenate([self.drone_history[t],
                                    self.ctrl_obs_history[t].flatten()])
                    for t in range(SEQ_LEN)
                ]).astype(np.float32)  # (3180,)

                predicted_future = self._predict_future(
                    self.drone_history, self.obs_history)  # (10, 9) or None

                if predicted_future is not None:
                    x_future = predicted_future[:, :9].flatten().astype(np.float32)
                else:
                    x_future = np.zeros(90, dtype=np.float32)

                x_ctrl = np.concatenate([x_past, x_future])  # (3270,)
            ...
            # ── Evasion blend ──────────────────────────────────────────────
            evade_vel, urgency, boundary_vel = self._compute_evasion(
                pos, vel, obs_all, dt)
            blend = np.clip(urgency * 2.5, 0.0, 1.0)

            # ── Predictive urgency boost from GRU ─────────────────────────
            if predicted_future is not None:
                pred_threat      = predicted_future[0, :3]
                pred_dist_to_obs = np.linalg.norm(
                    obs_all[:, :3] - pred_threat, axis=1).min()
                if pred_dist_to_obs < EVASION_RADIUS * 1.2:
                    urgency = min(1.0, urgency + 0.2)
                    blend   = np.clip(urgency * 2.5, 0.0, 1.0)

            # ── Gain interpolation cruise ↔ combat ─────────────────────────
            u                = blend
            ...
            vx_des_w = ((1.0 - blend) * vx_des
                        + blend * evade_vel[0] * eff_evade_speed
                        + boundary_vel[0])
            vy_des_w = ((1.0 - blend) * vy_des
                        + blend * evade_vel[1] * eff_evade_speed
                        + boundary_vel[1])
            ...
            if pos[2] < FLOOR + 0.8:
                base_thr += (FLOOR + 0.8 - pos[2]) * 15.0 / MAX_THRUST
